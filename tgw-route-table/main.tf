...
resource "aws_ec2_transit_gateway_route" "this" {
  ...
  destination_cidr_block         = var.transit_gateway_static_routes[count.index]["cidr_block"]
  transit_gateway_attachment_id  = var.transit_gateway_static_routes[count.index]["attachment_id"]
  transit_gateway_route_table_id = aws_ec2_transit_gateway_route_table.this.id
}
